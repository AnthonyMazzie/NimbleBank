...
/* Returns 1 if it is the first entry of the file, else returns a new ID corresponding to the number of lines in the file */
int getNewAccountID()
{
...
void login(void)
{
    ...
    {
        fputs("\nThere are no users in database, try registering first.\n", stderr);
        exit(1);
    }
    else
    {
...
int balanceCheck(int accountID)
{
    printf("\n-------------------------");
    printf("\nBALANCE CHECK");
    // printf("\nVector size: %ld", userBankAccountVector.size());
